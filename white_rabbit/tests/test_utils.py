...
def test_filter_events_per_month_and_week():
    event_01 = {
    ...
    event_04 = {
        'project_id': 2, 'name': 'Project1', 'subproject_name': None,
    ...
    event_month_01 = filter_events_per_time_period(events, month=datetime(2022, 1, 1))
    assert len(event_month_01) == 1
    assert event_month_01[0] == event_01

    events_month_3 = filter_events_per_time_period(events, month=datetime(2022, 3, 1))
    assert len(events_month_3) == 3

    events_week_10 = filter_events_per_time_period(events, week=datetime(2022, 3, 9))
    assert len(events_week_10) == 2
    assert events_week_10[0] == event_03
    assert events_week_10[1] == event_04

    event_week_11 = filter_events_per_time_period(events, week=datetime(2022, 3, 13))
...
def test_count_number_days_spent_per_project_id():
    event_01 = {
        'project_id': 1, 'name': 'Project1', 'subproject_name': None,
        'start_datetime': datetime(2022, 1, 1),
        'duration': 6.0,
    ...
    assert number_days_spent_per_project_id == {
        1: {'days_spent': 1.5, 'duration': 9.0, 'category': '',
            'events': [
                {'project_id': 1, 'name': 'Project1', 'subproject_name': None,
                 'start_datetime': datetime(2022, 1, 1, 0, 0),
                 'duration': 6.0, 'category': 'category'},
                {'project_id': 1, 'name': 'Project1', 'subproject_name': None,
                 'start_datetime': datetime(2022, 2, 10, 0, 0),
                 'duration': 3.0, 'category': 'category1'}]},
        3: {'days_spent': 1.0, 'duration': 6.0, 'category': '',
            'events': [
                {'project_id': 3, 'name': 'Project2', 'subproject_name': None,
                 'start_datetime': datetime(2022, 3, 10, 0, 0),
                 'duration': 6.0, 'category': 'category1'}]}}


def test_group_events_per_category():
    category = 'category1'
    ...
    events = [
        ...
        {
            'project_id': 2, 'name': 'Project2', 'subproject_name': None, 'start_datetime': datetime(2022, 1, 1),
            'duration': 6.0, 'category': category_2
        },
        {
            'project_id': 2, 'name': 'Project2', 'subproject_name': None, 'start_datetime': datetime(2022, 3, 1),
            'duration': 3.0, 'category': ""
        },
    ]
    ...
    assert events_per_category == {
        ...
        'category2': [
            {'project_id': 2, 'name': 'Project2', 'subproject_name': None,
             'start_datetime': datetime(2022, 1, 1, 0, 0), 'duration': 6.0, 'category': 'category2'}
        ],
        '': [
            {'project_id': 2, 'name': 'Project2', 'subproject_name': None,
             'start_datetime': datetime(2022, 3, 1, 0, 0), 'duration': 3.0, 'category': ''}
        ]
    }
...
def test_group_events_by_day():
    ...
    assert events_per_day == {
        date(2022, 1, 1): [
            {'project_id': 3, 'name': 'Project_3', 'subproject_name': None,
             'start_datetime': datetime(2022, 1, 1, 0, 0),
             'category': 'CLIENT', 'duration': 1.0}],
        date(2022, 1, 2): [
            {'project_id': 2, 'name': 'Project_2', 'subproject_name': None,
             'start_datetime': datetime(2022, 1, 2, 0, 0),
             'duration': 3.0, 'category': 'PRO_BONO'}],
        date(2022, 3, 1): [
            {'project_id': 4, 'name': 'Project_4', 'subproject_name': None,
